...
/* The keyboard scancodes.
**
** These are the PS/2 set 1 codes DirectInput reports, and they are not an implementation detail
** here: KeyDefs.h maps its own KEY_ enum onto them and the game writes the result into the key
** bindings, so the numbers are a file format.  A wrong one is a key that stops working rather than
** a compile error, which is why they are written out rather than guessed at.
**
** The second name in each pair is the spelling this tree uses where it differs from Microsoft's.
*/
